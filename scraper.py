...
class IndeedScraper:
    ...
    def __init__(self):
        ...
        self.job_limit = 50     # Max jobs per page

        self.start = 0
        self.end = 0        # Total jobs from query
    ...
    def main(self):
        ...
        while True:
            query = f"{self.indeed}jobs?q={self.job}&l={self.city}&2C+{self.state}&limit={self.max_job}&explvl={self.level}&start={self.start}"
            ...
            with open("jobs.csv", mode="a") as csv_file:
                ...
                for result in results:
                    ...
                    link = make_tiny(f"{URL}{title.attrs.get('href')[1:]}")
                    job["Link"] = link
